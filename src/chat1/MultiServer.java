...
public class MultiServer {
	...
	public static void main(String[] args) {
		...
		try {
			/*
			 9999번으로 포트번호를 설정한 후 서버인스턴스를 생성하고 클라이언트의 접속을 기다린다.
			 (Scanner 인스턴스와 같이 대기 상태가 된다.)
			 */
			...
			System.out.println(socket.getInetAddress() + "(클라이언트)의" + 
					socket.getPort() + "포트를 통해" + socket.getLocalAddress() 
					+ "(서버)의" + socket.getLocalPort() + "포트로 연결되었습니다.");
			
			// 서버 -> 클라이언트 쪽으로 메세지를 전송하기 위한 출력스트림 생성
