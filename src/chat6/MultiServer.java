...
public class MultiServer {
	...
	public MultiServer() {
		/*
		 클라이언트의 이름과 접속 시 생성한 출력스트림을 저장할 HashMap 인스턴스 생성
		 */
	...
	public void sendAllMsg(String name, String msg) {
		...
		while (it.hasNext()) {
			try {
				// 각 클라이언틔 PrintWriter 인스턴스를 추출한다.
	...
	class MultiServerT extends Thread {
		...
		public void run() {
			...
			try {
				/*
				 클라이언트가 보내는 최초메세지는 대화명이므로 접속에 대한 부분을 출력하고
				 Echo 한다.
				 */
				// 클라이언트의 이름을 읽어온다.
				name = in.readLine();
				...
				/*
				 클라이언트가 접속을 종료하면 Socket예외가 발생하게 되어 finally절로
				 진입하게 된다. 이 때 "대화명"을 통해 Map에서 클라이언트의 정보를 제거한다.
				 */
